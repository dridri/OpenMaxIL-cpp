	...
	, mHandle( nullptr )
{
	if ( mDebugCallback == nullptr ) {
		mDebugCallback = &Component::DefaultDebugCallback;
	}
	...
	for ( PortInit n : input_ports ) {
		...
		if ( mVerbose ) {
			mDebugCallback( 1, "Adding input port { %d, %d }\n", p.nPort, p.type );
		}
	...
	for ( PortInit n : output_ports ) {
		...
		p.bufferDataAvailable = false;
		if ( mVerbose ) {
			mDebugCallback( 1, "Adding output port { %d, %d }\n", p.nPort, p.type );
		}
...
Component::~Component()
{
	...
	for ( OMX_U8* buf : mAllocatedBuffers ) {
		...
		mDebugCallback( 1, "Freeing buffer %p\n", buf );
		vcos_free( buf );
	}
...
OMX_ERRORTYPE Component::SetState( const State& st_, bool wait )
{
			...
			if ( p.nPort != 0 ) {
				...
				if ( p.bTunneled and not p.bEnabled ) {
					...
					if ( err != OMX_ErrorNone ) {
						mDebugCallback( 0, "[%s]SendCommand( OMX_CommandPortEnable, %d ) failed\n", mName.c_str(), p.nPort );
						return err;
					}
					if ( wait ) {
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s]SendCommand( OMX_CommandStateSet, %d ) failed\n", mName.c_str(), st );
		return err;
	}

	if ( wait ) {
		if ( mVerbose ) mDebugCallback( 1, "[%s]Wating state to be %d\n", mName.c_str(), st );
		OMX_ERRORTYPE r;
		OMX_STATETYPE st_wait = OMX_StateInvalid;
		do {
			if ( ( r = OMX_GetState( mHandle, &st_wait ) ) != OMX_ErrorNone ) {
				mDebugCallback( 0, "[%s]omx_block_until_state_changed: Failed to get component state : %d", mName.c_str(), r );
				return r;
			}
			if ( st_wait != st ) {
				usleep( 10000 );
...
int Component::InitComponent()
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "OMX_GetHandle(%s) failed : %X\n", mName.c_str(), err );
		return -1;
	} else if ( mVerbose ) {
		mDebugCallback( 1, "OMX_GetHandle(%s) completed\n", mName.c_str() );
	}
...
{
	mDebugCallback( 1, "[%s] DisableProprietaryTunnels( %d, %d )\n", mName.c_str(), port, disable );
	if ( port == 0 ) {
	...
	mOutputPorts[port].bDisableProprietary = true;
	...
	OMX_ERRORTYPE ret = SetParameter( OMX_IndexParamBrcmDisableProprietaryTunnels, &tunnels );
	...
	if ( dynamic_cast<Camera*>(this) != nullptr ) {
		...
		camera->setResolution( camera->width(port), camera->height(port), port );
	}
	return ret;
}
...
void Component::fillInput( uint16_t port, uint8_t* pBuf, uint32_t len, bool corrupted, bool eof )
{
	...
	if ( buffer ) {
		...
		if ( err != OMX_ErrorNone ) {
			mDebugCallback( 0, "Component::fillInput : EmptyThisBuffer error : 0x%08X (bufSize : %d ; corrupted : %d)\n", (uint32_t)err, len, corrupted );
			fflush( stdout );
		}
	}
}
...
int32_t Component::getOutputData( uint16_t port, uint8_t* pBuf, bool wait )
{
	...
	if ( mVerbose ) {
		mDebugCallback( 1, "[%s] Component::getOutputData() : mHandle->FillThisBuffer()\n", mName.c_str() );
	}
	OMX_ERRORTYPE err = ((OMX_COMPONENTTYPE*)mHandle)->FillThisBuffer( mHandle, buffer );
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] Component::getOutputData : FillThisBuffer error : 0x%08X\n", mName.c_str(), (uint32_t)err );
		return err;
	}

	if ( buffer ) {
		if ( mVerbose ) {
			mDebugCallback( 1, "[%s] Component::getOutputData() locking...\n", mName.c_str() );
		}
		std::unique_lock<std::mutex> locker( mDataAvailableMutex );

		if ( mVerbose ) {
			mDebugCallback( 1, "mOutputPorts[%d].bufferDataAvailable = %d\n", port, mOutputPorts[port].bufferDataAvailable );
		}
		if ( not mOutputPorts[port].bufferDataAvailable and wait ) {
			if ( mVerbose ) {
				mDebugCallback( 1, "[%s] Component::getOutputData() : Waiting...\n", mName.c_str() );
			}
			mDataAvailableCond.wait( locker );
		} else if ( not wait ) {
			locker.unlock();
			if ( mVerbose ) {
				mDebugCallback( 1, "[%s] Component::getOutputData() unlocked\n", mName.c_str() );
			}
...
OMX_ERRORTYPE Component::SendCommand( OMX_COMMANDTYPE Cmd, OMX_U32 nParam1, OMX_PTR pCmdData )
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] SendCommand %X failed : %X\n", mName.c_str(), Cmd, err );
	} else if ( mVerbose ) {
		mDebugCallback( 1, "[%s] SendCommand %X completed\n", mName.c_str(), Cmd );
	}
...
OMX_ERRORTYPE Component::GetParameter( OMX_INDEXTYPE nParamIndex, OMX_PTR pComponentParameterStructure )
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] GetParameter %s failed : %X\n", mName.c_str(), name, err );
	} else if ( mVerbose and nParamIndex != OMX_IndexParamPortDefinition ) {
		mDebugCallback( 1, "[%s] GetParameter %s completed\n", mName.c_str(), name );
	}
...
OMX_ERRORTYPE Component::SetParameter( OMX_INDEXTYPE nIndex, OMX_PTR pComponentParameterStructure )
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] SetParameter %s failed : %X\n", mName.c_str(), name, err );
	} else if ( mVerbose and nIndex != OMX_IndexParamPortDefinition ) {
		mDebugCallback( 1, "[%s] SetParameter %s completed\n", mName.c_str(), name );
	}
...
OMX_ERRORTYPE Component::GetConfig( OMX_INDEXTYPE nIndex, OMX_PTR pComponentConfigStructure )
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] GetConfig %X failed : %X\n", mName.c_str(), nIndex, err );
	} else if ( mVerbose ) {
		mDebugCallback( 1, "[%s] GetConfig %X completed\n", mName.c_str(), nIndex );
	}
...
OMX_ERRORTYPE Component::SetConfig( OMX_INDEXTYPE nIndex, OMX_PTR pComponentConfigStructure )
{
	...
	if ( err != OMX_ErrorNone ) {
		mDebugCallback( 0, "[%s] SetConfig %X failed : %X\n", mName.c_str(), nIndex, err );
	} else if ( mVerbose ) {
		mDebugCallback( 1, "[%s] SetConfig %X completed\n", mName.c_str(), nIndex );
	}
	return err;
}
...
OMX_ERRORTYPE Component::SetupTunnel( uint16_t port_output, Component* next, uint16_t port_input, Port* port_copy )
{
	...
	if ( port_input == 0 ) {
		...
		if ( mVerbose ) {
			mDebugCallback( 1, "%p[%s] SetupTunnel detected input port : %d\n", mHandle, mName.c_str(), port_input );
		}
	}
...
{
	if ( mVerbose ) {
		mDebugCallback( 1, "%p[%s]->AllocateBuffers( %p, %d, %d )\n", mHandle, mName.c_str(), buffer, port, enable );
	}
	...
	for ( i = 0; i < portdef.nBufferCountActual; i++ ) {
		...
		if ( mVerbose ) {
			mDebugCallback( 1, "Allocated a buffer of %d bytes\n", size );
		}
		ret = OMX_UseBuffer( mHandle, end, port, nullptr, size, buf );
		if ( ret != OMX_ErrorNone ) {
			mDebugCallback( 0, "OMX_UseBuffer error 0x%08X ! (state : %d)\n", ret, (int)state() );
			*buffer = list;
			return ret;
		}
...
{
	mDebugCallback( 1, "Component::DestroyAll()\n" );

	// First, stop cameras
	for( auto comp : mComponents ) {
		if ( dynamic_cast< Camera* >( comp ) != nullptr ) {
			mDebugCallback( 1, "Stopping Camera %s\n", comp->mName.c_str() );
			dynamic_cast< Camera* >( comp )->SetCapturing( false );
			mDebugCallback( 1, "Camera stopped\n" );
		}
	}
	usleep( 1000 * 100 );
...
// 	for ( iter = mComponents.rbegin(); iter != mComponents.rend(); iter++ ) {
	for ( iter = mComponents.begin(); iter != mComponents.end(); iter++ ) {
		mDebugCallback( 1, "Stopping component %s\n", (*iter)->mName.c_str() );
	...
	for ( riter = mComponents.rbegin(); riter != mComponents.rend(); riter++ ) {
		mDebugCallback( 1, "Deleting component %s\n", (*riter)->mName.c_str() );
		delete (*riter);
		mDebugCallback( 1, "Component delete ok\n" );
	}
	mComponents.clear();
	...
	for ( OMX_U8* buf : mAllAllocatedBuffers ) {
		vcos_free( buf );
		mDebugCallback( 1, "Freed static buffer %p\n", buf );
	}
...
{
	if ( mVerbose ) mDebugCallback( 1, "Waiting port %d to be %d\n", nPortIndex, bEnabled );
	OMX_ERRORTYPE r;
	OMX_PARAM_PORTDEFINITIONTYPE portdef;
	OMX_INIT_STRUCTURE(portdef);
	...
	while ( i++ == 0 || portdef.bEnabled != bEnabled ) {
		if ( ( r = OMX_GetParameter( mHandle, OMX_IndexParamPortDefinition, &portdef ) ) != OMX_ErrorNone ) {
			mDebugCallback( 0, "[%s]omx_block_until_port_changed: Failed to get port definition : %d", mName.c_str(), r );
		}
...
{
	if ( mVerbose ) mDebugCallback( 1, "Waiting state to be %d\n", state );
	OMX_ERRORTYPE r;
	OMX_STATETYPE st = (OMX_STATETYPE)-1;
	OMX_U32 i = 0;
	while ( i++ == 0 || st != state ) {
		if ( ( r = OMX_GetState( mHandle, &st ) ) != OMX_ErrorNone ) {
			mDebugCallback( 0, "[%s]omx_block_until_state_changed: Failed to get handle state : %d", mName.c_str(), r );
		}
		if ( st != state ) {
			usleep(10000);
		}
	...
	if ( mVerbose and event != OMX_EventCmdComplete ) {
		if ( event == OMX_EventError ) {
			if ( (int)data1 != OMX_ErrorSameState ) {
				mDebugCallback( 0, "[%s]OMX Error %X\n", mName.c_str(), data1 );
			}
		} else {
			mDebugCallback( 1, "Event on %p (%s) type %X [ %d, %d, %p ]\n", mHandle, mName.c_str(), event, data1, data2, eventdata );
		}
	}
	return OMX_ErrorNone;
}
...
{
	if ( mVerbose ) {
		mDebugCallback( 1, "Component::FillBufferDone on %p[%s] (%p)\n", mHandle, mName.c_str(), buf );
	}
	for ( auto p : mOutputPorts ) {
		if ( p.second.buffer == buf ) {
			if ( mVerbose ) {
				mDebugCallback( 1, "[%s] Component::FillBufferDone locking [%d]...\n", mName.c_str(), p.second.nPort );
			}
			std::unique_lock<std::mutex> locker( mDataAvailableMutex );
			p.second.bufferDataAvailable = true;
			mDataAvailableCond.notify_all();
			if ( mVerbose ) {
				mDebugCallback( 1, "[%s] Component::FillBufferDone unlocked [%p.bufferDataAvailable:%d]\n", mName.c_str(), p.second.bufferDataAvailable );
			}
// 			break;
		}
	}
...
{
	mDebugCallback( 1, "CopyPort( %s->%d, %s->%d\n", from->pParent->name().c_str(), from->nPort, to->pParent->name().c_str(), to->nPort );

	if ( dynamic_cast<VideoRender*>(to->pParent) != nullptr ) {
...
void Component::print_def( OMX_PARAM_PORTDEFINITIONTYPE def )
{
	mDebugCallback( 1, "Port %u: %s %u/%u %u %u %s,%s,%s %ux%u %ux%u @%ufps %u\n",
		def.nPortIndex,
